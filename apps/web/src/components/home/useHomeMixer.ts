...
import type { HeroSpotlight } from "./sections/HeroSection";
import type { MarqueeItem } from "./sections/HomeMarqueeSection";
import {
  createTrackPlayback,
  fallbackBackdrop,
  fallbackCover,
...
export default function useHomeMixer(
  ...
  // 4. Mapeos a ViewModels (Memorizados para evitar recálculos en cada render de React)
    ...
    () => [
      {
        ...
        href: "/music",
        items: featuredTracks.slice(3, 7).map((track) => ({
          id: track.id,
          title: track.title,
          image: track.cover ?? fallbackCover,
